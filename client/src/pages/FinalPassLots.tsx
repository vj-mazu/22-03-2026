...
const compactModalGridStyle: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'minmax(0, 1.3fr) minmax(118px, 0.7fr) minmax(118px, 0.8fr)', gap: '6px', marginBottom: '6px' };
const compactSplitInputStyle: React.CSSProperties = { display: 'flex', gap: '4px', flexWrap: 'wrap' };
const compactNarrowFieldStyle: React.CSSProperties = { maxWidth: '132px' };
const compactMiniFieldStyle: React.CSSProperties = { maxWidth: '124px' };
...
const normalizeOfferVersions = (offering: any): OfferVersionData[] => {
  ...
  return offering.offerVersions
    ...
    .sort((left: any, right: any) => {
      return getOfferIndex(left.key) - getOfferIndex(right.key);
    });
};
...
const getFinalPassCollectedByDisplay = (entry: any, supervisors?: { username: string; fullName?: string | null }[]) => {
  // Always show the ORIGINAL sample collector, not the resample assigned user
  const originalCollector = getOriginalCollector(entry);
  if ((entry as any).sampleGivenToOffice && originalCollector) {
    return `${getCreatorLabel(entry)} / ${getCollectorLabel(originalCollector, supervisors)}`;
  }
  if (originalCollector) return getCollectorLabel(originalCollector, supervisors);
  return '-';
};
...
const FinalPassLots: React.FC<FinalPassLotsProps> = ({ entryType, excludeEntryType }) => {
  ...
  const visibleOfferKeys = Array.from(new Set([
    ...
    currentOfferKey
  ])).sort((left, right) => getOfferIndex(left) - getOfferIndex(right));
                      ...
                      <table style={{ width: '100%', minWidth: tableMinWidth, borderCollapse: 'collapse', fontSize: '11px', tableLayout: 'fixed', border: '1px solid #000' }}>
                          ...
                          <tr style={{ backgroundColor: '#1a237e', color: 'white' }}>
                            {isRiceMode ? (
                              ...
                              <>
                                ...
                                <th style={{ border: '1px solid #000', padding: '3px', fontWeight: '700', fontSize: '12px', textAlign: 'left',   whiteSpace: 'normal', wordBreak: 'break-word', width: '8%' }}>Sample Report By</th>
                                ...
                                <th style={{ border: '1px solid #000', padding: '3px', fontWeight: '700', fontSize: '12px', textAlign: 'center', whiteSpace: 'normal', wordBreak: 'break-word', width: '5%' }}>Offer Rate</th>
                        ...
                        <tbody>
                          {orderedEntries.map((entry, index) => {
                            ...
                            const slNo = index + 1;
                            const originalType = (() => { const e = entry as any; if (e.originalEntryType && e.entryType === 'LOCATION_SAMPLE' && entry.lotSelectionDecision === 'FAIL') { return e.originalEntryType === 'DIRECT_LOADED_VEHICLE' ? 'RL' : e.originalEntryType === 'LOCATION_SAMPLE' ? 'LS' : 'MS'; } return entry.entryType === 'DIRECT_LOADED_VEHICLE' ? 'RL' : entry.entryType === 'LOCATION_SAMPLE' ? 'LS' : entry.entryType === 'RICE_SAMPLE' ? 'RS' : 'MS'; })();
                            const isResampleEntry = entry.lotSelectionDecision === 'FAIL' || Number(entry.qualityReportAttempts || 0) > 1 || (Array.isArray((entry as any).qualityAttemptDetails) && (entry as any).qualityAttemptDetails.length > 1);
                            const rowType = isResampleEntry ? `${originalType}/LS` : originalType;
                                  ...
                                  <>
                                    ...
                                    <td style={{ border: '1px solid #000', padding: '3px', textAlign: 'center' }}>
                                      {(isAdmin || isManager) ? (
                                        ...
                                        <button onClick={() => handleOpenOfferModal(entry)} style={{ padding: '3px 7px', backgroundColor: o?.offerBaseRateValue ? '#3498db' : '#2196F3', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer' }}>
                                          {formatOfferBadge(o)}
                                        </button>
                                      </div>
                                    ) : <span>{o?.offerBaseRateValue ? toNumberText(o.offerBaseRateValue) : '-'}</span>}
                                    </td>
                                    <td style={{ border: '1px solid #000', padding: '3px', textAlign: 'center' }}>
                                      {isAdmin ? (
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', marginBottom: '6px', alignItems: 'center' }}>
                      <input type="text" inputMode="decimal" value={offerData.offerBaseRateValue}
                        ...
                        style={{ ...inputStyle, flex: 1, minWidth: '100px' }} placeholder="Rate" />
                      <select value={offerData.baseRateType}
                        onChange={e => setOfferData({ ...offerData, baseRateType: e.target.value })}
                        style={{ ...inputStyle, width: '100px', flexShrink: 0, cursor: 'pointer', fontSize: '11px' }} required>
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', marginBottom: '6px', alignItems: 'center' }}>
                      ...
                      <select value={finalData.baseRateType}
                        ...
                        style={{ ...inputStyle, width: '100px', flexShrink: 0, cursor: 'pointer', fontSize: '11px' }}>
                        <option value="PD_WB">PD/WB</option>
                        <option value="PD_LOOSE">PD/Loose</option>
                        <option value="MD_WB">MD/WB</option>
                        <option value="MD_LOOSE">MD/Loose</option>
